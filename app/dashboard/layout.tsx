  ...
  return (
    ...
    <div className={`min-h-screen ${bg} ${isDetailPage ? '' : 'flex'}`}>
      ...
      {!isDetailPage && (
        <>
          ...
          <button 
            ...
            className="lg:hidden fixed top-4 left-4 z-[60] p-2 bg-zinc-800 rounded-xl text-white"
          >
            {mobileMenuOpen ? (
          ...
          {/* Sidebar */}
          <aside className={`
            w-64 ${card} border-r ${border} flex flex-col fixed top-0 left-0 h-full z-50
          ...
          `}>
            <div className={`p-4 md:p-6 border-b ${border}`}>
              <Link href="/" className="flex items-center gap-2">
                <div className="w-8 h-8 md:w-10 md:h-10 bg-indigo-500 rounded-xl flex items-center justify-center">
            ...
            <nav className="flex-1 p-4 space-y-2">
              <Link href="/dashboard" onClick={() => setMobileMenuOpen(false)} className={`flex items-center gap-3 px-4 py-3 ${isActive("/dashboard") ? "bg-indigo-500/20 text-indigo-400 rounded-xl font-medium" : `${textMuted} hover:bg-zinc-700/50 rounded-xl transition-colors`}`}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <Link href="/dashboard/landings" onClick={() => setMobileMenuOpen(false)} className={`flex items-center gap-3 px-4 py-3 ${isActive("/dashboard/landings") ? "bg-indigo-500/20 text-indigo-400 rounded-xl font-medium" : `${textMuted} hover:bg-zinc-700/50 rounded-xl transition-colors`}`}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <Link href="/dashboard/boutiques" onClick={() => setMobileMenuOpen(false)} className={`flex items-center gap-3 px-4 py-3 ${isActive("/dashboard/boutiques") ? "bg-indigo-500/20 text-indigo-400 rounded-xl font-medium" : `${textMuted} hover:bg-zinc-700/50 rounded-xl transition-colors`}`}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <Link href="/dashboard/analytics" onClick={() => setMobileMenuOpen(false)} className={`flex items-center gap-3 px-4 py-3 ${isActive("/dashboard/analytics") ? "bg-indigo-500/20 text-indigo-400 rounded-xl font-medium" : `${textMuted} hover:bg-zinc-700/50 rounded-xl transition-colors`}`}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <Link href="/dashboard/settings" onClick={() => setMobileMenuOpen(false)} className={`flex items-center gap-3 px-4 py-3 ${isActive("/dashboard/settings") ? "bg-indigo-500/20 text-indigo-400 rounded-xl font-medium" : `${textMuted} hover:bg-zinc-700/50 rounded-xl transition-colors`}`}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className={`flex-1 ${isDetailPage ? '' : 'lg:ml-64'}`}>
        <header className={`${card} border-b ${border} px-4 md:px-6 lg:px-8 py-3 md:py-4 flex items-center justify-between`}>
            <div className="ml-10 lg:ml-0">
              <h1 className={`text-lg md:text-xl lg:text-2xl font-bold ${text}`}>{getPageTitle()}</h1>
              <p className={`text-xs md:text-sm ${textMuted} hidden sm:block`}>{t("welcome")}, {user?.name}</p>
            </div>
            <div className="flex items-center gap-2 md:gap-4">
              <div className="relative" ref={notifRef}>
                ...
                {showNotifications && (
                  <div className={`absolute right-0 top-full mt-2 w-72 md:w-80 ${card} border ${border} rounded-xl shadow-xl z-50 overflow-hidden`}>
                    <div className={`p-3 md:p-4 border-b ${border}`}>
                        ...
                        notifications.map((notif) => {
                          ...
                          return (
                            <Link
                              ...
                              className={`flex items-start gap-3 p-3 md:p-4 hover:bg-zinc-700/50 transition-colors border-b ${border} last:border-0`}
                            >
              ...
              <div className="flex items-center gap-2 md:gap-3">
                <div className="w-8 h-8 md:w-10 md:h-10 bg-indigo-500 rounded-full flex items-center justify-center text-white font-semibold text-sm md:text-base">
