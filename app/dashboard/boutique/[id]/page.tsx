...
export default function BoutiqueDetailPage() {
  ...
  const [boutique, setBoutique] = useState<Landing | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [boutiqueData, ordersData] = await Promise.all([
          getLanding(boutiqueId),
          getOrders()
        ]);
        setBoutique(boutiqueData.landing);
        setSettingsForm({
          name: boutiqueData.landing?.name || '',
          slug: boutiqueData.landing?.slug || '',
          brandName: boutiqueData.landing?.content?.brandName || boutiqueData.landing?.name || '',
        });
        const filteredOrders = (ordersData.orders || []).filter((o: Order) => o.landingSlug === boutiqueData.landing?.slug);
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      await updateLanding(boutiqueId, updates);
      ...
      <header className={`${card} border-b ${border} px-6 py-4`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => router.push('/dashboard/boutiques')} className={`p-2 hover:bg-zinc-700 rounded-lg ${textMuted}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <div>
              <h1 className={`text-2xl font-bold ${text}`}>{boutique?.name || 'Loading...'}</h1>
              <p className={textMuted}>{boutique?.type} • {boutique?.isPublished ? 'En ligne' : 'Hors ligne'}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Link 
              href={`/editor/${boutique?.type}-boutique?id=${boutiqueId}&type=boutique`}
        ...
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 ${
              activeTab === 'orders' ? 'bg-pink-500 text-white' : 'bg-zinc-700 text-zinc-300 hover:bg-zinc-600'
            }`}
          >
            Commandes
            {stats.pending > 0 && (
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${
              activeTab === 'analytics' ? 'bg-pink-500 text-white' : 'bg-zinc-700 text-zinc-300 hover:bg-zinc-600'
            }`}
          >
            Analytique
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${
              activeTab === 'settings' ? 'bg-pink-500 text-white' : 'bg-zinc-700 text-zinc-300 hover:bg-zinc-600'
            }`}
          >
            Paramètres
          </button>
            ...
            <div className="flex gap-4">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-zinc-800 border border-zinc-700 rounded-xl text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-pink-500"
                />
              </div>
              <select
                value={filterStatus}
                onChange={(e) => setFilterStatus(e.target.value)}
                className="px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-pink-500"
              >
              ...
              getFilteredOrders().map((order) => {
                ...
                return (
                  <div
                    ...
                    className={`${card} rounded-xl p-4 border ${order.status === 'pending' ? 'border-red-500/50' : border} cursor-pointer hover:border-pink-500/50 transition-colors`}
                  >
                    <div className="flex items-center gap-4">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              <div className={`${card} rounded-xl p-5 border ${border}`}>
                ...
                <p className={`text-3xl font-bold ${text}`}>{boutique?.views || 0}</p>
              </div>
              <div className={`${card} rounded-xl p-5 border ${border}`}>
                <div className="flex items-center gap-3 mb-3">
            ...
            <div className={`${card} rounded-xl p-6 border ${border}`}>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setGraphPeriod('day')} className={`px-3 py-1 text-sm rounded-lg ${graphPeriod === 'day' ? 'bg-pink-500 text-white' : 'bg-zinc-700 text-zinc-300 hover:bg-zinc-600'}`}>7 Jours</button>
                  <button onClick={() => setGraphPeriod('month')} className={`px-3 py-1 text-sm rounded-lg ${graphPeriod === 'month' ? 'bg-pink-500 text-white' : 'bg-zinc-700 text-zinc-300 hover:bg-zinc-600'}`}>6 Mois</button>
                  <button onClick={() => setGraphPeriod('year')} className={`px-3 py-1 text-sm rounded-lg ${graphPeriod === 'year' ? 'bg-pink-500 text-white' : 'bg-zinc-700 text-zinc-300 hover:bg-zinc-600'}`}>5 Ans</button>
                </div>
              </div>
                  ...
                  return (
                    <div key={idx} className="flex-1 flex flex-col items-center">
                      <div className="w-full flex flex-col items-center justify-end h-full gap-2">
                        <div className={`w-full rounded-t-lg bg-gradient-to-t from-pink-600 to-pink-400`} style={{ height: `${Math.max(heightPercent, data.orders > 0 ? 10 : 2)}%` }}></div>
            ...
            <div className={`${card} rounded-xl p-6 border ${border}`}>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={settingsForm.name} onChange={(e) => setSettingsForm({ ...settingsForm, name: e.target.value })} className="w-full px-4 py-3 bg-zinc-700 border border-zinc-600 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-pink-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-zinc-400 mb-2">URL</label>
                  <input type="text" value={settingsForm.slug} onChange={(e) => setSettingsForm({ ...settingsForm, slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') })} className="w-full px-4 py-3 bg-zinc-700 border border-zinc-600 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-pink-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-zinc-400 mb-2">Nom de marque</label>
                  <input type="text" value={settingsForm.brandName} onChange={(e) => setSettingsForm({ ...settingsForm, brandName: e.target.value })} className="w-full px-4 py-3 bg-zinc-700 border border-zinc-600 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-pink-500" />
                </div>
                <button onClick={handleSaveSettings} disabled={savingSettings} className={`w-full px-4 py-3 font-medium rounded-xl ${savingSettings ? 'bg-zinc-600 text-zinc-400' : 'bg-pink-500 hover:bg-pink-600 text-white'}`}>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(Object.keys(STATUS_LABELS) as OrderStatus[]).map(status => (
                    <button key={status} onClick={() => handleStatusUpdate(selectedOrder.id, status)} disabled={updatingStatus !== null || selectedOrder.status === 'deleted'}
                      className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${selectedOrder.status === status ? `${STATUS_COLORS[status].bg} ${STATUS_COLORS[status].text} border` : 'bg-zinc-700 text-zinc-300 hover:bg-zinc-600'} ${updatingStatus || selectedOrder.status === 'deleted' ? 'opacity-50 cursor-not-allowed' : ''}`}>
              ...
              {selectedOrder.status !== 'deleted' && (
                <div className="pt-4 border-t border-zinc-700">
                  <button onClick={() => handleDeleteOrder(selectedOrder.id)} className="w-full px-4 py-3 bg-red-500 hover:bg-red-600 text-white font-medium rounded-xl">Supprimer la commande</button>
                </div>
              )}
            </div>
          </div>
