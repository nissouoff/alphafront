...
export default function LandingOrdersPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const landingPages = landingsResult.landings.filter((l: Landing) => l.isLanding === true);
      setLandings(landingPages);
  ...
  const filteredOrders = orders.filter(order => {
    const isLandingOrder = landings.some(l => l.slug === order.landingSlug);
    ...
    return isLandingOrder && matchesStatus && matchesLanding && matchesSearch;
  });
          ...
          <div>
            ...
            {landingsOpen && (
              <div className="ml-4 mt-1 space-y-1">
                ...
                <Link href="/dashboard/landings/orders" className={`flex items-center justify-between px-4 py-2 text-sm bg-indigo-500/20 text-indigo-400 rounded-lg font-medium`}>
          ...
          <div>
            ...
            {boutiquesOpen && (
              <div className="ml-4 mt-1 space-y-1">
                ...
                <Link href="/dashboard/boutiques/orders" className={`flex items-center gap-2 px-4 py-2 text-sm ${styles.textMuted} hover:bg-zinc-700/50 rounded-lg transition-colors`}>
                  <span>📋</span> {t("orders")} ({t("boutiques")})
                </Link>
              </div>
            )}
          </div>
      ...
      <main className="flex-1 overflow-hidden">
        <header className={`${styles.card} border-b ${styles.border} px-8 py-4`}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className={`text-2xl font-bold ${styles.text}`}>{t("orders")} - {t("landings")}</h1>
          </div>
          <p className={styles.textMuted}>{t("landings")} - {filteredOrders.length} {t("orders").toLowerCase()}</p>
        </header>
            ...
            <>
              ...
              <div className="flex flex-wrap gap-4 mb-6">
                <div className="flex-1 min-w-[200px]">
                  <input
                    ...
                    className={`w-full px-4 py-2 ${styles.card} border ${styles.border} rounded-xl focus:ring-2 focus:ring-indigo-500`}
                  />
                </div>
                <select
                  value={landingFilter}
                  onChange={(e) => setLandingFilter(e.target.value)}
                  className={`px-4 py-2 ${styles.card} border ${styles.border} rounded-xl focus:ring-2 focus:ring-indigo-500`}
                >
                  <option value="all">{t("landings")}</option>
                  {landings.map(l => (
              ...
              {filteredOrders.length === 0 ? (
                <div className={`${styles.card} rounded-2xl p-12 border ${styles.border} text-center`}>
                  ...
                  <p className={styles.textMuted}>Les commandes de vos landing pages apparaîtront ici</p>
                </div>
              ) : (
                  ...
                  {filteredOrders.map((order) => {
                    ...
                    return (
                      <div
                        ...
                        className={`${styles.card} rounded-xl p-4 border ${isPending ? 'border-red-500/50 bg-red-500/5' : styles.border} cursor-pointer hover:border-indigo-500/50 transition-colors`}
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-xs text-indigo-400">{getLandingName(order.landingSlug)}</span>
                            </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4">
          <div className="bg-zinc-800 rounded-2xl max-w-md w-full p-6 border border-zinc-700">
            ...
            <button
              ...
              className="w-full mt-3 px-4 py-3 border border-zinc-600 text-zinc-400 hover:bg-zinc-700 rounded-xl transition-colors"
            >
              Annuler
            </button>
            <button
